...
// ROCm path
  ...
  const auto base_entry = block_entry_start + threadIdx.x * ENTRIES_PER_THREAD;
 ...
 public:
  static torch::autograd::variable_list forward_impl(
    ...
    auto dispatch_cumsum = [&](auto vec_tag) {
      ...
      if (rocm_grid_size > 1) {
        block_flags =
            at::zeros({rocm_grid_size}, lengths.options().dtype(at::kInt));
                  ...
                  AT_DISPATCH_INDEX_TYPES(
                      ...
                      [&] {
                        FBGEMM_LAUNCH_KERNEL(
                            ...
                            rocm_grid_size > 1 ? block_flags.data_ptr<int>()
                                               : nullptr,
                            rocm_grid_size > 1 ? block_sums.data_ptr<offset_t>()
                                               : nullptr);
                      });
                });
          });
